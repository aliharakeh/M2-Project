...
def tweet_scroller(url):
    ...
    while True:
        ...
        if count == 10 or newHeight == lastHeight:
            break
        else:
...
def scrapper(url):
    ...
    for i in soup.find_all('li', {"data-item-type": "tweet"}):
        user = (i.find('span', {'class': "username js-action-profile-name"}).get_text() if i.find('span', {
            'class': "username js-action-profile-name"}) is not None else "")
        ...
        popular = (i.find('div', {'class': "js-tweet-text-container"}).get_text().replace('\n', '') if i.find('div', {
            'class': "js-tweet-text-container"}) is not None else "")
        text = (i.p.get_text().replace('\n', '') if i.p is not None else "")
        popular_text = [i + ':' + j if len(dgt.findall(popular)) != 0 else '' for i, j in
                        zip(wrd.findall(crp.sub('', popular)), dgt.findall(popular))]
        ...
        blog_dict = {
            ...
            "like_fave": (
                int(''.join(fave.findall(str(popular_text)))) if len(fave.findall(str(popular_text))) > 0 else ''),
            "share_retweet": (
                int(''.join(retweet.findall(str(popular_text)))) if len(retweet.findall(str(popular_text))) > 0 else '')
        }
    ...
    with io.open(file_out, "w", encoding="utf-8") as csvfile:
        ...
        for i in blog_list:
            if len(i['blog_text']) > 0:
                newrow = i['header'], i['url'], i['user'], i['date'], i["popular"], i["blog_text"], i["like_fave"], i[
                    "share_retweet"]
